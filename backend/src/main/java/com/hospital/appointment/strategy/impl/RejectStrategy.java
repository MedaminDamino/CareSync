...
@Component
public class RejectStrategy implements AppointmentStatusStrategy {
    ...
    public void execute(Appointment appointment, User currentUser) {
        // Validation: must be in PENDING state to reject
        ...
        if (currentUser.getRole() == Role.DOCTOR) {
            if (appointment.getDoctor() == null || 
                ...
                throw new BadRequestException("Doctors can only reject their own appointments.");
            }
        } else if (currentUser.getRole() != Role.ADMIN) {
            throw new BadRequestException("Only doctors and admins can reject appointments.");
        }
